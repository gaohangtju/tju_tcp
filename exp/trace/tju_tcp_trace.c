...
static uint32_t local_ip_by_host(void){
    ...
    if(strcmp(host,"server")==0) return inet_network("172.17.0.3");
    return inet_network("172.17.0.2");
}
static uint32_t remote_ip_by_host(void){
    char host[8]; gethostname(host, 8);
    if(strcmp(host,"server")==0) return inet_network("172.17.0.2");
...
/* ===== 实验插桩：仅用于第三阶段拥塞窗口观测，不改变协议行为 ===== */
#define CWND_TRACE(e, tag) do { \
    struct timespec _ts; clock_gettime(CLOCK_MONOTONIC, &_ts); \
    fprintf(stderr, "[CWND] %.6f %s cwnd=%u ssthresh=%u flight=%u snd_nxt=%u snd_una=%u\n", \
            _ts.tv_sec + _ts.tv_nsec/1e9, (tag), (unsigned)(e)->cwnd, (unsigned)(e)->ssthresh, \
            (unsigned)((e)->snd_nxt - (e)->snd_una), (unsigned)(e)->snd_nxt, (unsigned)(e)->snd_una); \
} while (0)
